    ...
    methods
        function obj = Ui(app)
            ...
            obj.function_list = {'Change Drive','Save all stacks', 'Plot all Gr', 'Plot all TimeStamps', 'Get Scale', 'Set Scales','Plot scales',...
                ...
                'Consolidate voids data','Visualize voids data','Create voids images','Plot movements - stack',...
                'Get movements - all stacks','Plot Void size dist - all stacks','Overlay eigenvectors'};
            obj.createUi();
        end
        ...
        function createUi(obj)
            ...
            obj.controls.imageNumber = uicontrol(obj.axesPanel, 'Style', 'text', ...
                'Units', 'normalized', 'Position', [0.7 0.01 0.1 0.08], ...
                'FontSize', 14);
            ...
            obj.info.N_info = uicontrol(obj.buttonPanel, 'Style', 'edit', ...
                'Units', 'normalized', 'Position', [0.15 0.87 0.1 0.04], 'String', '48');
            obj.info.f_info = uicontrol(obj.buttonPanel, 'Style', 'edit', ...
                'Units', 'normalized', 'Position', [0.4 0.87 0.1 0.04], 'String', '10');
            obj.info.i_info = uicontrol(obj.buttonPanel, 'Style', 'edit', ...
                'Units', 'normalized', 'Position', [0.65 0.87 0.1 0.04], 'String', '4');
        ...
        function create_menu(obj)
            % RDF menu
            rdfMenu = uimenu(obj.fig, 'Text', 'RDF');
            uimenu(rdfMenu, 'Text', 'Plot all Gr', ...
                'MenuSelectedFcn', @(src,evt) obj.app.rdf.plot_all_gr());
            uimenu(rdfMenu, 'Text', 'Local number density', ...
                'MenuSelectedFcn', @(src,evt) obj.app.rdf.plot_local_number_density());

            % Timer menu
            timerMenu = uimenu(obj.fig, 'Text', 'Timer');
            uimenu(timerMenu, 'Text', 'Plot all TimeStamps', ...
                'MenuSelectedFcn', @(src,evt) obj.app.timer.plot_all_timestamps());
            uimenu(timerMenu, 'Text', 'Average timeStamps', ...
                'MenuSelectedFcn', @(src,evt) obj.app.timer.average_timeStamps());
            uimenu(timerMenu, 'Text', 'Plot durations', ...
                'MenuSelectedFcn', @(src,evt) obj.app.timer.plot_stack_durations());

            % Trial menu
            trialMenu = uimenu(obj.fig, 'Text', 'Trial');
            uimenu(trialMenu, 'Text', 'Get Scales', ...
                'MenuSelectedFcn', @(src,evt) obj.app.trial.get_scales());
            uimenu(trialMenu, 'Text', 'Set Scales', ...
                'MenuSelectedFcn', @(src,evt) obj.app.trial.set_bd());
            uimenu(trialMenu, 'Text', 'Plot scales', ...
                'MenuSelectedFcn', @(src,evt) obj.app.trial.plot_scales());
            uimenu(trialMenu, 'Text', 'Set stack empty status', ...
                'MenuSelectedFcn', @(src,evt) obj.app.trial.set_stack_empty_or_not());
            uimenu(trialMenu, 'Text', 'Set all Empty or Not', ...
                'MenuSelectedFcn', @(src,evt) obj.app.trial.set_all_empty_or_not());
            uimenu(trialMenu, 'Text', 'Set all Jam or Not', ...
                'MenuSelectedFcn', @(src,evt) obj.app.trial.set_all_jam_or_not());
            uimenu(trialMenu, 'Text', 'Plot movements - stack', ...
                'MenuSelectedFcn', @(src,evt) obj.app.trial.plot_movement());
            uimenu(trialMenu, 'Text', 'Get movements - all stacks', ...
                'MenuSelectedFcn', @(src,evt) obj.app.trial.get_all_movements());

            % Steinhardt menu
            steinhardtMenu = uimenu(obj.fig, 'Text', 'Steinhardt');
            uimenu(steinhardtMenu, 'Text', 'Find all nhood', ...
                'MenuSelectedFcn', @(src,evt) obj.app.steinhardt.find_all_nhood());
            uimenu(steinhardtMenu, 'Text', 'Find all Si6', ...
                'MenuSelectedFcn', @(src,evt) obj.app.steinhardt.find_all_psi());
            uimenu(steinhardtMenu, 'Text', 'Get LBOOP', ...
                'MenuSelectedFcn', @(src,evt) obj.app.steinhardt.get_LBOOP());
            uimenu(steinhardtMenu, 'Text', 'Get all LBOOP', ...
                'MenuSelectedFcn', @(src,evt) obj.app.steinhardt.get_all_LBOOP());
            uimenu(steinhardtMenu, 'Text', 'Plot LBOOP for all N', ...
                'MenuSelectedFcn', @(src,evt) obj.app.steinhardt.plot_LBOOP_per_chain());
            uimenu(steinhardtMenu, 'Text', 'Plot avg LBOOP for all N', ...
                'MenuSelectedFcn', @(src,evt) obj.app.steinhardt.plot_avg_LBOOP_per_chain());
            uimenu(steinhardtMenu, 'Text', 'Plot BOOP', ...
                'MenuSelectedFcn', @(src,evt) obj.app.steinhardt.plot_avg_BOOP());
            uimenu(steinhardtMenu, 'Text', 'Superpose all LBOOP', ...
                'MenuSelectedFcn', @(src,evt) obj.app.steinhardt.superpose_all_LBOOP());

            % Voids menu
            voidsMenu = uimenu(obj.fig, 'Text', 'Voids');
            uimenu(voidsMenu, 'Text', 'Detect voids - stack', ...
                'MenuSelectedFcn', @(src,evt) obj.app.voids.detect_voids_stack());
            uimenu(voidsMenu, 'Text', 'Detect voids - all stacks', ...
                'MenuSelectedFcn', @(src,evt) obj.app.voids.detect_voids_all_stacks());
            uimenu(voidsMenu, 'Text', 'Analyze voids - stack', ...
                'MenuSelectedFcn', @(src,evt) obj.app.voids.analyze_stack_voids());
            uimenu(voidsMenu, 'Text', 'Analyze voids - all stacks', ...
                'MenuSelectedFcn', @(src,evt) obj.app.voids.analyze_all_stacks_voids());
            uimenu(voidsMenu, 'Text', 'Consolidate voids data', ...
                'MenuSelectedFcn', @(src,evt) obj.app.voids.consolidate_voids_data());
            uimenu(voidsMenu, 'Text', 'Visualize voids Characteristics', ...
                'MenuSelectedFcn', @(src,evt) obj.app.voids.visualize_voids_characteristics());
            uimenu(voidsMenu, 'Text', 'Plot voids size distribution', ...
                'MenuSelectedFcn', @(src,evt) obj.app.voids.plot_size_distribution());
            uimenu(voidsMenu, 'Text', 'Create void images', ...
                'MenuSelectedFcn', @(src,evt) obj.app.voids.create_void_images());
            uimenu(voidsMenu, 'Text', 'Plot Void size dist - all stacks', ...
                'MenuSelectedFcn', @(src,evt) obj.app.voids.plot_void_size_distr_over_time());
            uimenu(voidsMenu, 'Text', 'Overlay eigenvectors', ...
            ...
            uimenu(voids_area_frac_Menu, 'Text', 'Plot Void area frac. over time - Stacks', ...
                'MenuSelectedFcn', @(src,evt) obj.app.voids.plot_void_area_fraction_over_time_stacks());
            uimenu(voids_area_frac_Menu, 'Text', 'Plot area fracs. over time', ...
            'MenuSelectedFcn', @(src,evt) obj.app.voids.plot_chain_and_void_area_fraction_vs_time_combined());  
            uimenu(voids_area_frac_Menu, 'Text', 'Plot area fracs. over time-avg.', ...
            'MenuSelectedFcn', @(src,evt) obj.app.voids.plot_chain_and_void_area_fraction_vs_time_averaged());  
            uimenu(voids_area_frac_Menu, 'Text', 'Plot anisotropy norm over time', ...
            'MenuSelectedFcn', @(src,evt) obj.app.voids.plot_anisotropy_norm_vs_time_combined());  
            uimenu(voids_area_frac_Menu, 'Text', 'Plot anisotropy norm over time-avg.', ...
            ...
            uimenu(anisotropyMenu, 'Text', 'Frame', ...
                'MenuSelectedFcn', @(src,evt) obj.app.voids.analyze_2d_fabric_frame(obj.app.current_image_idx));
            uimenu(anisotropyMenu, 'Text', 'Stack', ...
                'MenuSelectedFcn', @(src,evt) obj.app.voids.analyze_2d_fabric_stack());
            uimenu(anisotropyMenu, 'Text', 'All stacks', ...
                'MenuSelectedFcn', @(src,evt) obj.app.voids.analyze_2d_fabric_stacks());
            uimenu(anisotropyMenu, 'Text', 'Plot Angle and anisotropy - Stack', ...
                'MenuSelectedFcn', @(src,evt) obj.app.voids.plot_anisotropy_and_angle_evolution_stack());
            uimenu(anisotropyMenu, 'Text', 'Plot Angle and anisotropy - Stacks', ...
                'MenuSelectedFcn', @(src,evt) obj.app.voids.plot_anisotropy_and_angle_evolution_stacks());
            uimenu(anisotropyMenu, 'Text', 'Create Videos - Stacks', ...
                'MenuSelectedFcn', @(src,evt) obj.app.voids.convert_image_dirs_to_videos());
                
            % Other/General menu
            generalMenu = uimenu(obj.fig, 'Text', 'General');
            uimenu(generalMenu, 'Text', 'Save all stacks', ...
                'MenuSelectedFcn', @(src,evt) obj.app.utils.save_all_stacks());
            uimenu(generalMenu, 'Text', 'Combine stacks', ...
